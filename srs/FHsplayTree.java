   ...
   protected FHs_treeNode<E> splay(FHs_treeNode<E> root, E x)
   {
      ...
      while (root != null)
      {
         ...
         if (compareResult < 0)
         {
            ...
            if (compareResult < 0) //todo: check sign >
            {
               newRoot = rotateWithRightChild(root);
               if (newRoot.rtChild == null)
               {
                  break;
               }
            }
            ...
            {
               leftTreeMax.rtChild = root; // todo: check rtChild
               //leftTreeMax = newRoot;
            }
            leftTreeMax = newRoot.rtChild;
            root = root.rtChild;
